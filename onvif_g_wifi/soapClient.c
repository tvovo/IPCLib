...
SOAP_SOURCE_STAMP("@(#) soapClient.c ver 2.8.21 2015-01-09 12:45:19 GMT")
...
{	struct SOAP_ENV__Fault_ex soap_tmp_SOAP_ENV__Fault_ex;
	...
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_SOAP_ENV__Fault_ex(soap, &soap_tmp_SOAP_ENV__Fault_ex, "SOAP-ENV:Fault-ex", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
...
{
	soap_default_SOAP_ENV__Fault_ex(soap, _param_8);
	soap_begin(soap);
	if (soap_begin_recv(soap)
	 || soap_envelope_begin_in(soap)
	 || soap_recv_header(soap)
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	soap_get_SOAP_ENV__Fault_ex(soap, _param_8, "SOAP-ENV:Fault-ex", NULL);
	if (soap->error == SOAP_TAG_MISMATCH && soap->level == 2)
		soap->error = SOAP_OK;
	if (soap->error
	 || soap_body_end_in(soap)
